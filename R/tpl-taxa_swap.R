...
TaxaSwap <- MarketDataFWF$proto(expr={
	...
	fields <- fields(
		...
		field('Código das curvas a termo', width(2), to_factor()),
		field('Código da taxa ', width(5), to_factor()),
		field('Descrição da taxa', width(15), to_factor()),
		...
		field('Taxa teórica', width(14),
			to_numeric(dec=7, sign='Sinal da taxa teórica')),
