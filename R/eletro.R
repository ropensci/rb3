...
Eletro <- MarketDataFWF$proto(expr={
	...
	fields <- fields(
		...
		field("Data de geração do arquivo", width=8),
		field("Tipo de negociação", width=2),
		field("Código da mercadoria", width=3),
		field("Código do mercado", width=1),
		field("Data-base", width=8),
		field("Data de vencimento", width=8),
		...
		field("Taxa média (swp)/prêmio médio (opç flex)", width=9),
		field("Sinal da taxa média/prêmio médio", width=1),
		field("Tipo de taxa média", width=1),
		field("Preço de exercício médio (opç flex)", width=22),
