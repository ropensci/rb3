...
BD_Arbit <- MarketDataFWF$proto(expr={
	...
	fields <- fields(
		...
		field('Complemento da transação', width(3), to_numeric()),
		field('Tipo de registro', width(2), to_numeric()),
		field('Data de geração do arquivo', width(8), to_date(format='%Y%m%d')),
		field('Tipo de negociação', width(2), to_factor(levels='PR', labels='Pregão')),
		field('Código da mercadoria', width(3)),
		field('Código do mercado', width(1)),
		field('Tipo da série', width(1), to_factor(
			levels=c('C', 'V', '*'),
			labels=c('Opção de Compra', 'Opção de Venda', 'Futuro')
		)),
		field('Série (opç)/vencimento (fut)', width(4), to_factor()),
		field('Hora de criação deste registro', width(6), to_time(format='%H%M%S')),
		field('Data de vencimento (fut/opç)', width(8), to_date(format='%Y%m%d')),
		field('Preço de exercício (opç)', width(13), to_numeric(dec='Número de casas decimais dos campos com *')),
		field('Valor do ponto ou tamanho do contrato', width(13), to_numeric(dec=7)),
		field('Volume do dia em R$', width(13), to_numeric()),
		field('Volume do dia em US$', width(13), to_numeric()),
		...
		field('Cotação da última oferta de compra do dia', width(8),
			to_numeric(dec='Número de casas decimais dos campos com *', sign='Sinal da cot.da última oferta de compra do dia')),
		...
		field('Cotação da última oferta de venda do dia', width(8),
			to_numeric(dec='Número de casas decimais dos campos com *', sign='Sinal da cot.da última oferta de venda do dia')),
		field('Sinal da cotação do primeiro negócio do dia', width(1)),
		field('Cotação do primeiro negócio do dia', width(8),
			to_numeric(dec='Número de casas decimais dos campos com *', sign='Sinal da cotação do primeiro negócio do dia')),
		field('Sinal da cotação do menor negócio do dia', width(1)),
		field('Cotação do menor negócio do dia', width(8),
			to_numeric(dec='Número de casas decimais dos campos com *', sign='Sinal da cotação do menor negócio do dia')),
		...
		field('Cotação média dos negócios do dia', width(8),
			to_numeric(dec='Número de casas decimais dos campos com *', sign='Sinal da cotação média dos negócios do dia')),
		...
		field('Cotação do último negócio do dia', width(8),
			to_numeric(dec='Número de casas decimais dos campos com *', sign='Sinal da cotação do último negócio do dia')),
		field('Hora do último negócio do dia', width(6),
			to_time(format='%H%M%S')),
		...
		field('Cotação do último negócio anterior', width(8),
			to_numeric(dec='Número de casas decimais dos campos com *', sign='Sinal da cotação do último negócio anterior')),
		field('Sinal da cotação de fechamento do dia', width(1)),
		field('Cotação de fechamento do dia', width(8),
			to_numeric(dec='Número de casas decimais dos campos com *', sign='Sinal da cotação de fechamento do dia')),
		field('Sinal da cotação ajuste (fut)', width(1)),
		field('Cotação ajuste (fut)', width(13),
			to_numeric(dec='Número de casas decimais dos campos com *', sign='Sinal da cotação ajuste (fut)')),
		field('Situação do ajuste do dia', width(1),
			to_factor(levels=c('S', 'A'), labels=c('Ajuste Final', 'Ajuste Corrigido'))),
		field('Sinal da cotação de ajuste do dia anterior (fut)', width(1)),
		field('Cotação de ajuste do dia anterior (fut)', width(13),
			to_numeric(dec='Número de casas decimais dos campos com *', sign='Sinal da cotação de ajuste do dia anterior (fut)')),
		field('Situação do ajuste do dia anterior', width(1),
			to_factor(levels=c('S', 'A'), labels=c('Ajuste Final', 'Ajuste Corrigido'))),
		field('Valor do ajuste por contrato', width(13), to_numeric(dec=2)),
		field('Volume exercido no dia em R$', width(13), to_numeric()),
		field('Volume exercido no dia em US$', width(13), to_numeric()),
		...
		field('Percentual de oscilação', width(8), to_numeric(dec=1)),
		...
		field('Valor da equivalência', width(8), to_numeric(dec=2)),
		field('Valor do dólar do dia anterior', width(13), to_numeric(dec=7)),
		field('Valor do dólar do dia atual', width(13), to_numeric(dec=7)),
		field('Valor do delta da opção (margem)', width(9), to_numeric(dec=7)),
		field('Qtd. saques até data de vencimento', width(5), to_numeric()),
		field('Qtd. dias corridos até data de vencimento', width(5), to_numeric()),
		field('Qtd. dias úteis até data de vencimento', width(5), to_numeric()),
		field('Vencimento do contrato-objeto', width(4), to_factor()),
		field('Margem para clientes normais', width(13), to_numeric(dec=2)),
		field('Margem para clientes hedgers', width(13), to_numeric(dec=2)),
		field('Data de início do período de entrega (agrícolas)', width(8), to_date(format='%Y%m%d')),
		...
		field('Canais de negociação permitidos', width(1),
			to_factor(levels=c('V', 'E', 'A'), labels=c('Viva-Voz', 'Eletrônico', 'Ambos'))),
		field('Referência deste resumo de negócios', width(4), to_factor(levels='MERC')),
		field('Data-limite para negociação', width(8), to_date(format='%Y%m%d')),
		field('Data de liquidação financeira', width(8), to_date(format='%Y%m%d')),
