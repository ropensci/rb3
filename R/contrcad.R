...
ContrCad <- MarketDataFWF$proto(expr={
	...
	fields <- fields(
		...
		field('Data de Referência', width=8, handler=to_date(format='%Y%m%d')),
		field('Código da Mercadoria', width=3, handler=to_factor()),
		...
		field('Indicador de Tipo de Opção', width=1, handler=to_factor(
			levels=c('C', 'V'),
			labels=c('Compra', 'Venda')
		)),
		field('Tipo de Opção', width=1, handler=to_factor(
			levels=c('A', 'E'),
			labels=c('Americana', 'Europeia')
		)),
		field('Data de Vencimento do Contrato', width=8, handler=to_date(format='%Y%m%d')),
		field('Data de Inicio de Negociação', width=8, handler=to_date(format='%Y%m%d')),
		field('Data de Inicio de Exercício', width=8, handler=to_date(format='%Y%m%d')),
		field('Data Limite de Negociação', width=8, handler=to_date(format='%Y%m%d')),
		field('Data Limite de Abertura de Posições', width=8, handler=to_date(format='%Y%m%d')),
		field('Preço de Exercício (Opções)', width=15, handler=to_numeric(dec='Número de Casas Decimais')),
		...
		field('Tipo de Cotação', width=1, handler=to_factor(
			levels=1:2,
			labels=c('Preço', 'Taxa')
		)),
		field('Tipo de Mercadoria', width=1, handler=to_factor(
			levels=1:3,
			labels=c('Financeiro', 'Agropecuário', 'Energia')
		)),
		field('Variação Mínima de Apregoação', width=15, handler=to_numeric(dec='Número de Casas Decimais')),
		field('Indicador de Opção com Ajuste', width=1),
		field('Indicador de Mercadoria Internacional', width=1),
		field('Código da Moeda', width=2, handler=to_factor(
			levels=1:2,
