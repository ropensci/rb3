...
Eletro <- MarketDataFWF$proto(expr={
	...
	fields <- fields(
		...
		field("Taxa média (swp)/prêmio médio (opç flex)", width(9), to_numeric(dec=4)),
		field("Sinal da taxa média/prêmio médio", width(1)),
		field("Tipo de taxa média", width(1), to_factor(
			levels=c('D', 'P'),
			labels=c('diária', 'no período')
		)),
		field("Preço de exercício médio (opç flex)", width(22), to_numeric(dec=7)),
