...
Premio <- MarketDataFWF$proto(expr={
	...
	fields <- fields(
		...
		field('Código da mercadoria', width(3), to_factor()),
		field('Tipo de mercado', width(1), to_factor(
			levels=1:5,
			labels=c('Disponível', 'Futuro', 'Opções sobre Disponível', 'Opções sobre Futuro', 'Termo')
		)),
		field('Série', width(4)),
		field('Tipo de opção', width(1), to_factor(
			levels=c('C', 'V'),
			labels=c('Compra', 'Venda')
		)),
		field('Modelo de opção', width(1), to_factor(
			levels=c('A', 'E'),
			labels=c('Americana', 'Europeia')
		)),
		field('Data de vencimento', width(8), to_date('%Y%m%d')),
		field('Preço de exercício', width(15), to_numeric(dec='Número de casas decimais')),
		field('Prêmio de referência', width(15), to_numeric(dec='Número de casas decimais')),
