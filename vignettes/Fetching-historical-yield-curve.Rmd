...
Each daily file holds every curve B3 publishes. We will work with three of them:
...
All curves are stored, so `yc_get()` can be filtered by `curve_name` later on; the helpers
`yc_brl_get()`, `yc_ipca_get()` and `yc_usd_get()` select PRE, DIC and DOC for us.
